...
DTYPES = {
    'tf32': (torch.float32, 'tf32'),
    'fp16': (torch.float16, 'ieee'),
    'bf16': (torch.bfloat16, 'ieee'),
}
...
def qkflash_kernel(
    ...
    for start_n in range(0, N_CTX, BLOCK_N):
        ...
        # INT8 MMA -> int32. dequant (q_scale*k_scale) and softmax base-2 scaling
        # (sm_scale*log2e) are fused into one multiply.
...
def qkflash_forward(
    ...
    target_dtype, precision = DTYPES[dtype]

    if scale is None:
        scale = 1.0 / (HEAD_DIM_Q ** 0.5)
    LOG2E = 1.44269504

    # Per-tensor symmetric int8 quantization for Q and K only. V stays float.
    ...
    v_f = value.to(target_dtype)
    o = torch.empty_like(v_f)

    qk_combined_scale = q_scale * k_scale * scale * LOG2E

    Z, H, N = q_i8.shape[0], q_i8.shape[1], q_i8.shape[2]
    ...
    def kernel():
        qkflash_kernel[grid](
            q_i8, k_i8, v_f, o,
            qk_combined_scale,
            *q_i8.stride(), *k_i8.stride(), *v_f.stride(), *o.stride(),
...
class qkflash:
    kernel = [qkflash_kernel]
    forward = qkflash_forward
